...
function update($querystr) {
	$('#tt').tabs('add',{
		title: '修改支出',
		href: '__URL__/income_update?' + $querystr,
		closable: true
	});
}
</script>
...
<div id="tt" class="easyui-tabs" style="width:auto; height:auto;">
	<div title="收入明细" style="padding: 20px;">
	...
	<table id="income" class="table">
		...
		<!-- 循环将数据输出 -->
		<?php if(is_array($incomeList)): $i = 0; $__LIST__ = $incomeList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
				<td><input type="checkbox"></td>
				<td><?php echo ($vo["item"]); ?></td>
				<td><?php echo ($vo["money"]); ?>元</td>
				<td><?php echo ($vo["time"]); ?></td>
				<td><a style="cursor:pointer" title="删除" onclick="del(<?php echo ($vo["id"]); ?>, 'income')"><img src="../Public/Images/del.png" border="0" /></a> 
	...
	<div title="搜索" style="padding: 20px;">
		...
		<select class="easyui-combobox" style="width:100px;" data-options="multiple:true, editable:false">
			<?php if(is_array($categoryList_income)): foreach($categoryList_income as $key=>$vo): ?><option value="<?php echo ($vo["item"]); ?>"><?php echo ($vo["item"]); ?></option><?php endforeach; endif; ?>
		</select>
		&nbsp;&nbsp;起止时间：
		...
		<br/>
		<form action="" method="get">
		<table id="incomeSearch" class="table">
			<tr>
				<th width="100px">类 别</th>
				<th width="100px">金 额</th>
				<th width="100px">时 间</th>
			...
			<!-- 循环将数据输出 -->
			<?php if(is_array($incomeSearchList)): $i = 0; $__LIST__ = $incomeSearchList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
					<td><?php echo ($vo["item"]); ?></td>
					<td><?php echo ($vo["money"]); ?>元</td>
					<td><?php echo ($vo["time"]); ?></td>
					<td><a href="__URL__/income_del?id=<?php echo ($vo["id"]); ?>" title="删除"><img src="../Public/Images/del.png" border="0" /></a> 
						<a style="cursor:pointer" title="修改" onclick="update('id=<?php echo ($vo["id"]); ?>&item=<?php echo ($vo["item"]); ?>&money=<?php echo ($vo["money"]); ?>&time=<?php echo ($vo["time"]); ?>')">
							<img src="../Public/Images/alter.png" border="0" /></a></td>
				</tr><?php endforeach; endif; else: echo "" ;endif; ?>
		</table>
